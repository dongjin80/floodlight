...
/**
 * Modules to send configuration commands to the COAP AP.
 * 
 * @author "Ashish Patro"
 */
public class ConfigManager {
	...
	public ConfigManager(IFloodlightProviderService floodlightProvider) {
	...
	public boolean SetSyncOffsetMicroseconds(int apId, long syncVal) {
		log.info("Updating the synchronization offset of " + syncVal + " to " + apId);
		String command = "bash /root/tdma_analysis/setsync_offset.sh " + syncVal;
		return SendCommand(apId, command);
	}
	...
	public boolean SendCommand(int apId, String command) {
		try {
			long switchId = MainStatsManager.GetDpIdFromAP(apId);
	...
	protected void sendWirelessConfigRequest(long switchId, String configMessage) {
		...
		if (sw != null) {
			...
			try {
				...
				log.error("Exception while sending config: " + e.getMessage());
